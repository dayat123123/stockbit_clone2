...
/// When user clicks BUY/Order, it navigates directly to the dedicated Orders tab with the stock symbol.
class WatchlistFeatureView extends StatefulWidget {
  final String? initialSymbol;

  const WatchlistFeatureView({super.key, this.initialSymbol});
...
class _WatchlistFeatureViewState extends State<WatchlistFeatureView> {
    ...
    if (widget.initialSymbol != null) {
      WidgetsBinding.instance.addPostFrameCallback((_) {
        context.read<WatchlistBloc>().add(
          SelectActiveStockEvent(widget.initialSymbol!),
        );
      });
    }
  }
  ...
  Widget build(BuildContext context) {
    return BlocListener<NavigationCubit, NavigationState>(
      listener: (context, navState) {
        if (navState.tab == AppNavTab.watchlist) {
          final sym = navState.getArg<String>('symbol');
          if (sym != null) {
      ...
      child: BlocBuilder<WatchlistBloc, WatchlistState>(
        builder: (context, state) {
          String activeSymbol = widget.initialSymbol ?? 'PACK';
          ...
          return Container(
            ...
            child: Column(
              children: [
                ...
                _buildTopStockSubHeader(activeSymbol, state),
  ...
  Widget _buildTopStockSubHeader(String symbol, WatchlistState state) {
    final isSaved =
        state is WatchlistLoadedState && state.isCurrentStockInWatchlist;
    ...
    return Container(
      ...
      child: Row(
        ...
        children: [
          // ── Left: Stock Emblem, Symbol & Ellipsized Company Name ───────────
          Expanded(
            child: Row(
              ...
              children: [
                ...
                // Company Name (Naturally shrinks and ellipsizes to prevent any overflow)
                Flexible(
                  child: Text(
          ...
          // ── Right: Standardized Header Actions Pinned to the Far Right Edge ───
          DesktopHeaderActions(
            activeSymbol: symbol,
            leadingActions: [
              _buildHeaderActionBtn(Icons.alarm, 'Alert'),
              // Interactive 1-Tap Watchlist Toggle Button
              InkWell(
                onTap: () {
                  context.read<WatchlistBloc>().add(
                    ToggleWatchlistStockEvent(symbol),
                  );
                  ScaffoldMessenger.of(context).showSnackBar(
                    SnackBar(
                      ...
                      content: Text(
                        ...
                        style: TextStyle(
                          color: isSaved ? AppColors.textPrimary : Colors.black,
                          fontWeight: FontWeight.bold,
                        ),
                      ),
    ...
    return Container(
      ...
      child: Row(
        ...
        children: [
          ...
          Text(
            label,
            style: const TextStyle(fontSize: 9.5, color: AppColors.textPrimary),
          ),
        ],
      ),
    );
  ...
  Widget _buildLeftWatchlistPanel(WatchlistState state, String activeSymbol) {
    ...
    final items = currentGroup.symbols
  ...
  Widget _buildCenterChartSection(String symbol) {
    return Column(
      children: [
        ...
        SizedBox(height: 140, child: _buildBottomPortfolioDrawer()),
      ],
    );
  }
    ...
    return Container(
      ...
      child: Column(
        children: [
          ...
          Expanded(flex: 5, child: _buildOrderbookDepthTable()),
          ...
          Expanded(flex: 4, child: _buildRunningTradesStream(symbol)),

          // Full-width Bottom Buy Button (Navigates to Orders tab with symbol)
          Padding(
