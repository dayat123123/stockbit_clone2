...
class OrderbookTopToolbar extends StatelessWidget {
  const OrderbookTopToolbar({super.key});
    ...
    return BlocBuilder<OrderbookBloc, OrderbookState>(
      builder: (context, state) {
        ...
        if (state is OrderbookLoadedState) {
          ...
          activeSymbol = state.activeWindow?.orderbook?.summary.symbol;
        }

        final tabs = state is OrderbookLoadedState ? state.tabs : [];
          ...
          child: Row(
            children: [
              ...
              ...tabs.asMap().entries.map((entry) {
                ...
                return InkWell(
                  onTap: () {
                    context.read<OrderbookBloc>().add(SelectWorkspaceTabEvent(index));
                  },
                  child: Container(
                    height: 42,
                    padding: const EdgeInsets.symmetric(horizontal: 14),
                    decoration: BoxDecoration(
                      color: isActiveTab ? AppColors.activeTabBg : AppColors.inactiveTabBg,
                      border: Border(
                        right: const BorderSide(color: AppColors.border, width: 1),
                        top: BorderSide(
                          color: isActiveTab ? AppColors.tabIndicator : Colors.transparent,
                          width: 2.5,
                        ),
                      ),
                    ),
                    child: Row(
                      ...
                      children: [
                        Icon(
                          Icons.grid_view,
                          size: 13,
                          color: isActiveTab ? AppColors.primaryGreen : AppColors.textSecondary,
                        ),
                        ...
                        Text(
                          ...
                          style: TextStyle(
                            ...
                            color: isActiveTab ? AppColors.textPrimary : AppColors.textSecondary,
                          ),
                        ),
                        if (tabs.length > 1) ...[
                          const SizedBox(width: 8),
                          InkWell(
                            onTap: () {
                              context.read<OrderbookBloc>().add(CloseWorkspaceTabEvent(index));
                            },
              ...
              IconButton(
                ...
                onPressed: () {
                  context.read<OrderbookBloc>().add(
                        const AddWorkspaceTabEvent(title: 'Orderbook Workspace'),
                      );
                },
              ),

              const SizedBox(width: 8),
              const VerticalDivider(color: AppColors.border, width: 1, indent: 8, endIndent: 8),
              const SizedBox(width: 8),
              ...
              Container(
                ...
                child: TextField(
                  ...
                  onSubmitted: (value) {
                    ...
                    if (sym.isNotEmpty && state is OrderbookLoadedState) {
                      final activeWin = state.activeWindow;
                      if (activeWin != null) {
                        context.read<OrderbookBloc>().add(
                              ChangeWindowSymbolEvent(windowId: activeWin.id, newSymbol: sym),
                            );
                      }
                    }
                  },
              ...
              if (activeSymbol != null)
                Container(
                  ...
                  child: Row(
                    children: [
                      const Icon(Icons.check_circle_outline, size: 11, color: AppColors.primaryGreen),
              ...
              // 3. Arrange Button: "Auto Arrange (Rapih)"
              ElevatedButton.icon(
                ...
                onPressed: () {
                  ...
                  final availableWidth = mediaSize.width - 54; // subtract sidebar
                  final availableHeight = mediaSize.height - 42 - 24; // subtract toolbars
                  context.read<OrderbookBloc>().add(
                        AutoArrangeWindowsEvent(Size(availableWidth, availableHeight)),
                      );
                },
                icon: const Icon(Icons.auto_awesome_mosaic_outlined, size: 13, color: AppColors.primaryGreen),
                label: const Text('Auto Arrange (Rapih)', style: TextStyle(fontSize: 10, fontWeight: FontWeight.bold)),
              ),

              const SizedBox(width: 8),

              // 4. Layout Mode Toggle: "Dinamis / Rapih"
              Tooltip(
                ...
                child: InkWell(
                  ...
                  child: Container(
                    ...
                    decoration: BoxDecoration(
                      ...
                      borderRadius: BorderRadius.circular(4),
                      border: Border.all(
                        color: isFreeFloating ? AppColors.badgePurple : AppColors.border,
                      ),
                    ),
                    child: Row(
                      children: [
                        Icon(
                          isFreeFloating ? Icons.pan_tool_alt_outlined : Icons.grid_on,
                          size: 13,
                          color: isFreeFloating ? AppColors.badgePurple : AppColors.textSecondary,
                        ),
                        ...
                        Text(
                          ...
                          style: TextStyle(
                            ...
                            color: isFreeFloating ? AppColors.badgePurple : AppColors.textPrimary,
                          ),
                        ),
                      ],
                    ),
              ...
              // 5. Grid Preset Selector Dropdown
              PopupMenuButton<Map<String, int>>(
                ...
                child: Container(
                  ...
                  child: Row(
                    children: [
                      const Icon(Icons.view_quilt_outlined, size: 13, color: AppColors.textSecondary),
                      const SizedBox(width: 5),
                      Text(
                ...
                onSelected: (val) {
                  ...
                  context.read<OrderbookBloc>().add(
                        SetGridPresetEvent(
                          rows: val['rows']!,
                          columns: val['cols']!,
                          canvasSize: Size(availableWidth, availableHeight),
                        ),
                      );
                },
              ),
              ...
              // 6. + Add Window Button
              IconButton(
                icon: const Icon(Icons.add_to_photos_outlined, size: 16, color: AppColors.primaryGreen),
  ...
  PopupMenuItem<Map<String, int>> _buildPresetItem(String label, int rows, int cols) {
